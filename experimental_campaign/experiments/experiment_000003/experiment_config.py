...
class ExperimentConfig:
    ...
    def get_pipeline_config(self):
        ...
        config.seed = int(0)
        config.n_random_batches = int(9)
        config.n_imgep_batches = int(36)
        config.batch_size = int(10)
    ...
    def get_goal_generator_config(self):
        ...
        if config.generator_type == "hypercube":
            ...
            config.IM_val_scaling = float()
            config.IM_grad_scaling = float()
            config.random_proba = float()
            config.flow_noise = float()
    ...
    def get_gc_intervention_optimizer_config(self):
        ...
        config.init_noise_std = jtu.tree_map(lambda low, high: float(0.1) * (high - low), config.low, config.high)

        if config.optimizer_type == "SGD":
            config.lr = jtu.tree_map(lambda low, high: float() * (high - low), config.low, config.high)
