...
class EvaluationConfig (ExperimentConfig):
    ...
    def get_select_representants_config(self):
        ...
        config.n_representants = int(<n_representants>)
        config.epsilon = 0.05
        config.n_trials = 500
        return config
    ...
    def get_pipeline_config(self):
        ...
        config.n_perturbations = int(<n_perturbations>)
    ...
    def get_perturbation_configs(self, T10, T90):
        ...
        test_tasks = {
            ...
            "noise_period": <noise_period_range>,
            "push_magnitude": <push_magnitude_range>,
            "push_number": <push_number_range>,
            "wall_length": <wall_length_range>,
            "wall_number": <wall_number_range>
        }
        
        deltaT = self.get_system_rollout_config().deltaT
        n_secs = self.get_system_rollout_config().n_system_steps * deltaT
        
        perturbation_min_duration = float(<perturbation_min_duration>)
        perturbation_max_duration = float(<perturbation_max_duration>)
            ...
            for var_val in test_task_var_range:
                ...
                if var_name.split("_")[0] == "noise":
                    ...
                    if var_name.split("_")[1] == "std":
                        n_noises = int((end-start)//float(<noise_period_default>))
                        ...
                        config.std = float(<noise_std_default>)
                    ...
                    if var_name.split("_")[1] == "magnitude":
                        ...
                        config.magnitude = float(<push_magnitude_default>)
                    ...
                    if var_name.split("_")[1] == "length":
                        ...
                        config.walls_length_range = [[<wall_length_default>, <wall_length_default>]] * var_val
